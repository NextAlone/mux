      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1">
          <div className="text-plan-mode mb-1 flex items-center gap-2 text-[13px] font-medium">
            ...
            <span>{t("Context May Exceed Model Limit")}</span>
          </div>
          <p className="text-foreground/80 text-[12px] leading-relaxed">
            {t("Current context (")}
            {formatTokens(props.warning.currentTokens)} {t(" tokens) may exceed the")}{" "}
            <span className="font-medium">{targetName}</span> {t("limit (")}
            {formatTokens(props.warning.targetLimit)}
            {t("). Consider compacting before sending.")}
          </p>
        </div>
        <button
          type="button"
          ...
          aria-label={t("Dismiss context limit warning")}
        >
