...
| Durable, resumable, reusable multi-agent orchestration | Workflow                                                                 | Read `workflow-authoring`; prefer declarative Markdown for ordered phases and JavaScript only for advanced control flow.                                                                               |
| Skill/instructions describing a multi-phase process    | One-off declarative `script_source` workflow                             | When prose reads like ordered phases and ships no packaged workflow, codify it as a Markdown template instead of executing every phase in-context.                                                     |
...
Choose a workflow when durability/reuse/resume or structured multi-agent phases justify the overhead. Ordered prose phases are a strong signal to codify a one-off declarative Markdown `script_source`; loops or parallel lanes require the JavaScript escape hatch. Either form executes the documented process more faithfully than ad-hoc in-context execution and survives interruption. Before authoring one:
