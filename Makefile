...
.PHONY: ensure-deps rebuild-native mux check-node-version
...
# Prune any node_modules tree (not just ./node_modules), isolated .worktrees,
# and the removed mobile/ prototype: upgraded checkouts may keep stale
# untracked leftovers there (e.g. CocoaPods .sh scripts under mobile/ios).
SHELL_SRC_FILES := $(shell find . -not \( -path '*/.git/*' -o -path '*/node_modules/*' -o -path './.worktrees/*' -o -path './mobile/*' -o -path './build/*' -o -path './dist/*' -o -path './release/*' -o -path './benchmarks/terminal_bench/.leaderboard_cache/*' \) -type f -name '*.sh' 2>/dev/null)
...
DOCKERFILES := $(shell find . -not \( -path '*/.git/*' -o -path '*/node_modules/*' -o -path './.worktrees/*' -o -path './mobile/*' -o -path './build/*' -o -path './dist/*' -o -path './release/*' -o -path './benchmarks/terminal_bench/.leaderboard_cache/*' \) -type f -name 'Dockerfile' 2>/dev/null)
