...
| `--runtime <runtime>` | `-r`  | Runtime: `local`, `worktree` for JJ Workspace, `ssh <host>`, or `docker <image>`                                              | `local`           |
...
- **`worktree`**: Runtime ID for an isolated JJ Workspace checkout under `~/.mux/src`. Useful for parallel work.
...
Run a durable [declarative Markdown workflow](/agents/workflows) or advanced JavaScript conductor from the command line without opening the desktop app. Workflows are addressed by explicit paths; there is no workflow discovery, `list`, or `show` command. Skill-packaged definitions use `skill://<skill-name>/workflow.md` (or legacy `workflow.js`), and local definitions use an explicit workspace-contained `.md` or `.js` path. `mux wf` is a shorthand alias for `mux workflow`.

<Note>
  `mux workflow` is experimental. Invoking the command implicitly enables the `dynamic-workflows`
  experiment for that invocation only. The initial CLI runner supports the `local` runtime only; JJ
  Workspace, SSH, Docker, and devcontainer workflow execution are blocked until their workflow
...
```bash
# Run a trusted declarative workflow template
mux workflow run ./workflows/three-stage.md --arg request="Add validation"

# Same as above, using the alias and machine-readable output
mux wf run ./workflows/three-stage.md --args-json '{"request":"Add validation"}' --json
```
...
| Option                       | Description                                                                        |
| ---------------------------- | ---------------------------------------------------------------------------------- |
| `--dir <path>` / `-d`        | Project directory. When omitted, Mux uses the repository root for the current cwd. |
...
Workspace workflow definitions are executable only after the project is trusted. If an explicit local `.md` or `.js` path points into an untrusted project, `mux workflow run` fails before executing repo-controlled workflow code. Linked jj workspaces inherit trust from the main repository path. Grant trust headlessly with [`mux trust`](#mux-trust).
...
```bash
# Trust the project containing the current directory (repository root)
mux trust
...
Run from a linked jj workspace (for example a mux workspace checkout), trust is recorded for the main repository path rather than the ephemeral workspace path. Review a repository before trusting it: trusted projects can execute repo-controlled code.
