...
| `--runtime <runtime>` | `-r`  | Runtime: `local`, `worktree` (JJ Workspace), `ssh <host>`, or `docker <image>`                                                | `local`           |
...
- **`worktree`**: Creates an isolated JJ Workspace checkout under `~/.mux/src`. Useful for parallel work.
