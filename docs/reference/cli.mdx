...
Workspace-file workflow scripts are executable only after the project is trusted. If an explicit local `.js` script path points into an untrusted project, `mux workflow run` fails before executing repo-controlled workflow code. Linked jj workspaces inherit trust from the main repository path. Grant trust headlessly with [`mux trust`](#mux-trust).
