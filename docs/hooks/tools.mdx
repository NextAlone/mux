...
<details>
<summary>fusion (8)</summary>

| Env var                                        | JSON path                       | Type    | Description                                |
| ---------------------------------------------- | ------------------------------- | ------- | ------------------------------------------ |
| `MUX_TOOL_INPUT_JUDGE_OVERRIDE_MODEL`          | `judgeOverride.model`           | string  | —                                          |
| `MUX_TOOL_INPUT_JUDGE_OVERRIDE_THINKING`       | `judgeOverride.thinking`        | enum    | —                                          |
| `MUX_TOOL_INPUT_PANEL_OVERRIDE_MODE`           | `panelOverride.mode`            | enum    | —                                          |
| `MUX_TOOL_INPUT_PANEL_OVERRIDE_MODELS_<INDEX>` | `panelOverride.models[<INDEX>]` | string  | —                                          |
| `MUX_TOOL_INPUT_PANEL_OVERRIDE_MODELS_COUNT`   | `panelOverride.models.length`   | number  | Number of elements in panelOverride.models |
| `MUX_TOOL_INPUT_PANEL_OVERRIDE_THINKING`       | `panelOverride.thinking`        | enum    | —                                          |
...
<details>
<summary>image_generate (5)</summary>

| Env var                               | JSON path              | Type   | Description                                                                |
| ------------------------------------- | ---------------------- | ------ | -------------------------------------------------------------------------- |
| `MUX_TOOL_INPUT_BACKGROUND`           | `background`           | enum   | Optional background mode. gpt-image-2 does not support transparent output. |
| `MUX_TOOL_INPUT_PROMPT`               | `prompt`               | string | The image generation or editing prompt.                                    |
| `MUX_TOOL_INPUT_QUALITY`              | `quality`              | enum   | Optional rendering quality. Use high for final assets and low for drafts.  |
| `MUX_TOOL_INPUT_REFERENCE_IMAGE_PATH` | `reference_image_path` | string | Optional local image path to use as editing/reference input.               |
| `MUX_TOOL_INPUT_SIZE`                 | `size`                 | enum   | Optional output size. Use auto unless the user asks for an orientation.    |
...
<details>
<summary>module_report (1)</summary>

| Env var               | JSON path | Type   | Description                                             |
| --------------------- | --------- | ------ | ------------------------------------------------------- |
| `MUX_TOOL_INPUT_PATH` | `path`    | string | Source file path, absolute or relative to the workspace |
...
<details>
<summary>read_enclosing (2)</summary>

| Env var               | JSON path | Type   | Description                                             |
| --------------------- | --------- | ------ | ------------------------------------------------------- |
| `MUX_TOOL_INPUT_LINE` | `line`    | number | 1-based source line to inspect                          |
| `MUX_TOOL_INPUT_PATH` | `path`    | string | Source file path, absolute or relative to the workspace |

</details>

<details>
<summary>read_symbol (4)</summary>

| Env var                     | JSON path   | Type   | Description                                                           |
| --------------------------- | ----------- | ------ | --------------------------------------------------------------------- |
| `MUX_TOOL_INPUT_KIND`       | `kind`      | enum   | Optional symbol kind used to disambiguate matching names              |
| `MUX_TOOL_INPUT_PATH`       | `path`      | string | Source file path, absolute or relative to the workspace               |
| `MUX_TOOL_INPUT_START_LINE` | `startLine` | number | Optional 1-based declaration line used to disambiguate matching names |
| `MUX_TOOL_INPUT_SYMBOL`     | `symbol`    | string | Exact symbol name or qualified name                                   |
...
<details>
<summary>send_follow_up (1)</summary>

| Env var                  | JSON path | Type   | Description                                     |
| ------------------------ | --------- | ------ | ----------------------------------------------- |
| `MUX_TOOL_INPUT_MESSAGE` | `message` | string | Message to send automatically on the next turn. |
...
| `MUX_TOOL_INPUT_ISOLATION`                     | `isolation`                   | enum    | Workspace isolation for the sub-agent. "fork" (the default) runs it in an isolated copy of this workspace created from committed state. "none" runs it directly in this workspace's checkout, sharing working-copy changes and skipping the fork + init overhead. Use "none" only for read-only analysis (e.g. the explore agent) or when you instruct the sub-agent to avoid editing shared files, since it can otherwise modify the same files concurrently. Omit to fork. |
...
| `MUX_TOOL_INPUT_WORKSPACE_BRANCH_NAME`         | `workspace.branchName`        | string  | Workspace/bookmark name to create or target. The field name is kept for API compatibility.                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `MUX_TOOL_INPUT_WORKSPACE_TRUNK_BRANCH`        | `workspace.trunkBranch`       | string  | Source bookmark or revision to fork from. The field name is kept for API compatibility.                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Env var                            | JSON path           | Type    | Description                                                                                                                            |
| ---------------------------------- | ------------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `MUX_TOOL_INPUT_DRY_RUN`           | `dry_run`           | boolean | When true, attempt to restore the task change in a temporary jj workspace and then discard it (does not modify the current workspace). |
| `MUX_TOOL_INPUT_EXPECTED_HEAD_SHA` | `expected_head_sha` | string  | When provided, refuse to apply unless the target jj change or Git commit ID matches this value.                                        |
| `MUX_TOOL_INPUT_FORCE`             | `force`             | boolean | When true, allow apply even if the patch was previously applied.                                                                       |
| `MUX_TOOL_INPUT_PROJECT_PATH`      | `project_path`      | string  | When provided, apply only the task-change artifact for this project path.                                                              |
| `MUX_TOOL_INPUT_TASK_ID`           | `task_id`           | string  | Child task ID whose task-change artifact should be applied                                                                             |
| `MUX_TOOL_INPUT_THREE_WAY`         | `three_way`         | boolean | Deprecated compatibility option; jj-native task apply ignores this value.                                                              |
...
| `MUX_TOOL_INPUT_ACTION`                                     | `action`                                       | enum    | Lifecycle action to perform: "archive" is the safe default, "delete_worktree" deletes the managed checkout after archive, and "remove" irreversibly deletes archived workspace metadata/session state. |
...
| `MUX_TOOL_INPUT_SCRIPT_PATH`       | `script_path`       | string  | Explicit workflow definition path. Prefer a declarative Markdown template such as "skill://review/workflow.md" or "./workflows/review.md"; advanced JavaScript conductors such as "skill://deep-research/workflow.js" remain supported.                                                                                                                                  |
| `MUX_TOOL_INPUT_SCRIPT_SOURCE`     | `script_source`     | string  | Inline declarative Markdown workflow template or advanced JavaScript conductor. Prefer Markdown for ordered phases; use JavaScript only for loops, branching, parallelism, or nested workflows. The compiled source is snapshotted for replay/resume.                                                                                                                    |
