...
Use `/fork` to clone a workspace with its full conversation history and UI state. The forked workspace gets a new checkout from the current workspace revision.
...
- Starts from the current workspace revision

**What's copied**: Conversation history, model selection, thinking level, auto-retry setting, UI mode (plan/exec), chat input text

**What's not copied**: Working-copy file changes (the new workspace starts from the source revision)
